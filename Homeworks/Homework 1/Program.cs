...
app.MapGet("/", () => "Домашняя работа 1\n\n/customs_duty?price=(float)[цена_посылки]&weight=(float)[вес_посылки] - стоимость налоговой пошлины;\n/current_date_time?lang=яз_ЯЗ - узнать сегодняшнюю дату в подробном формате");
...
string GetDuty(float? price, float? weight)
{
    try
    {
        return $"Налоговая пошлина\nНалоговая пошлина на вашу посылку составляет {(price = price < 200 && weight < 30 ? 0f : price / 100 * 15)} евро.";
    }
    catch(Exception ex)
    {
        return ex.Message;
