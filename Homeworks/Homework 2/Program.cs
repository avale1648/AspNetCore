...
app.MapGet("/catalogue/{id}", (int id) => catalogue.Read(id));
app.MapGet("/catalogue",() => catalogue.ReadAll());
app.MapPut("/catalogue/{id}", (int id, Item item) => catalogue.Update(id, item)) ;
app.MapDelete("/catalogue/{id}", (int id) => catalogue.Delete(id));
...
app.MapGet("/read", (int id) => catalogue.Read(id));
app.MapGet("/read_all", () => catalogue.ReadAll());
app.MapPost("/update", (int id, Item item) => catalogue.Update(id, item));
app.MapPost("/delete", (int id) => catalogue.Delete(id));
